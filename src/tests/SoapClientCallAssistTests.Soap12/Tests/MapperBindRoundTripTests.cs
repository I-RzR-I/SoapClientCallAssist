...
public sealed class MapperBindRoundTripTests
{
    ...
    private XElement Emit(MapperBindProduct product)
    {
        ...
        Assert.IsNotNull(
            root.Element(XName.Get("product", MapperBindNs.Contract)),
            "The emitted operation element must carry the parameter element.");
    ...
    {
        Assert.IsNotNull(actual, "Nothing was bound for the product.");
        ...
        Assert.IsNotNull(actual.Detail, "The nested complex member was not bound.");
        ...
        Assert.IsNotNull(actual.LocationIds, "The collection of primitives was not bound.");
        CollectionAssert.AreEqual(expected.LocationIds.ToArray(), actual.LocationIds.ToArray());

        Assert.IsNotNull(actual.Lines, "The collection of complex items was not bound.");
    ...
    public void RoundTrip_EmittedTreeIndentedByTheServiceFormatter_StillRebindsEveryMember_Test()
    {
        ...
        Assert.IsTrue(indented.Contains("\n"), "The fixture must really be indented.");
    ...
    public void RoundTrip_NullReferenceMemberOnATypeWithNoInitializer_ComesBackNull_Test()
    {
        ...
        Assert.IsNull(
            emitted.Descendants(XName.Get("Code", MapperBindNs.Contract)).FirstOrDefault(),
            "A null member must be omitted, not emitted as nil.");
    ...
    public void RoundTrip_EmptyCollection_ComesBackNullAndIsOutsideTheSubset_Test()
    {
        ...
        Assert.IsNull(
            emitted.Descendants(XName.Get("LocationIds", MapperBindNs.Contract)).FirstOrDefault(),
            "An empty collection must be omitted by the emitter for this pin to mean anything.");
        ...
        MapperBindAssert.Succeeded(result);
        Assert.IsNull(
            result.Response.Product.LocationIds,
            "An empty collection does not survive a round trip; it returns as null.");
    }
    ...
    public void RoundTrip_CollectionHoldingANullItem_LosesThatItemAndIsOutsideTheSubset_Test()
    {
        ...
        Assert.AreEqual(
            2,
            emitted.Descendants(XName.Get("Line", MapperBindNs.Contract)).Count(),
            "A null item must be skipped by the emitter for this pin to mean anything.");
